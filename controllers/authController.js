...
import userRepositories, {
    getUserById,
} from "../repositories/userRepositories.js";
import sessionsRepository, {
    getSession,
} from "../repositories/sessionsRepositories.js";

export async function signUp(req, res) {
    const newUser = req.body;

    try {
        const checkUserEmail = await userRepositories.getUserEmail(
            newUser.email
        );
...
export async function login(req, res) {
    ...
    try {
        ...
        if (
            checkUserEmail.rowCount > 0 &&
            bcrypt.compareSync(user.password, checkUserEmail.rows[0].password)
        ) {
            const tokenData = {
            ...
            await sessionsRepository.createSessions(
                checkUserEmail.rows[0].id,
                token
            );
